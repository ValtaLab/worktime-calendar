...
(() => {
  ...
  /* 統計膠囊字級「按情況」自適應：內容寬到必須折行時（如手機直向四枚膠囊），
     逐步縮小字級（每次 0.5px，下限 11px）讓膠囊排回同一行；
     縮到下限仍放不下（極窄機＋超長數字）就回預設字級、保持換行——資訊完整優先。
     觸發時機：每次渲染統計、視窗 resize。 */
  function fitStatsRow() {
    const box = el.monthStats;
    if (!box) return;
    box.style.fontSize = '';
    const pills = box.querySelectorAll('.stat-pill');
    if (pills.length < 2) return;
    const onOneRow = () => [...pills].every((p) => p.offsetTop === pills[0].offsetTop);
    if (onOneRow()) return;
    let size = parseFloat(getComputedStyle(box).fontSize);
    for (size -= .5; size >= 11; size -= .5) {
