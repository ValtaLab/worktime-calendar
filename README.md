...
| **工時、加班、半夜分行** | 月曆上工數、加班時數、半夜加班時數各佔一行或一塊，不並排 |
| **描述優先** | 格子內先顯示描述，時數在下方 |
| **三組成型分組** | 每格分成三組：**工時描述＋工數**、**加班描述＋加班時數**、**半夜加班描述＋半夜加班時數**，每組都是描述在上、數字緊接在下 |
...
| **收入總計** | 設定日薪、加班時薪、半夜加班時薪後，月曆下方自動計算本月收入（工數 × 日薪 ＋ 加班時數 × 加班時薪 ＋ 半夜加班時數 × 半夜加班時薪） |
...
| --- | --- | --- | --- | --- | --- | --- |
| ≤ 480px | **網格線** | 10px | 52px | 9.5px | 2 | 由內容決定 |
| ≥ 700px | 卡片 | 28px | 89px | 14px | 2 | 130px（由內容決定） |
| ≥ 1000px | 卡片 | 40px | 126px | 18px | 2 | 276px |
| ≥ 1400px | 卡片 | 56px | 177px | 19px | 2 | 308px |

> 手機版描述限 2 行（而非 3 行）：加入半夜加班這第三組後，若三個描述都放到 3 行，
> 單日格子會長到近 200px，整個月就失去「一眼掃描」的意義。

### 格子內容結構

每一格最多分成**三組**，每組都是「描述在上、數字緊接在下」：
...
.day-groups
 ...
 │   └─ .day-units.ot-units   加班時數色塊（橘）
 └─ .day-group.night-group  半夜加班組
     ├─ .day-desc.night-text   半夜加班描述（紫字）
     └─ .day-units.night-units 半夜加班時數色塊（紫）
```

沒有內容的組不會產生（例如沒填加班就只有工時一組、只填半夜就只有半夜一組）。
`.day-groups` 用 `margin-top: auto` 整組貼齊格子底部；組內 `gap` 小、組間 `gap` 大，
讓三組資料在視覺上分得開。角落另有對應色點（`.badge-dot` / `.badge-dot.ot` / `.badge-dot.night`）。

**色塊**滿版橫跨格子寬度、貼齊底部：手機版為連續的底色資料列，桌機版另有 `space-between`
佈局（左側數字、右側「加班」／「半夜」標籤）。

**列高的估算基準**：一個格子最擠的情況是「工時描述 2 行 + 加班描述 2 行 + 半夜描述 2 行
+ 三個色塊 + 兩段組間距」。1000px 以上用固定列高（`minmax(Npx, 1fr)`），N 必須放得下這個
最擠情境，否則 `.day` 的 `overflow: hidden` 會把下面的色塊裁掉。加入第三組後實測
1000px 需 274px、1400–1600px 需 306px，故分別取 276px 與 308px。
（三組全滿的最擠情境另有自動化測試覆蓋：`/tmp/test_night.py` 的第 11 項。）

> 改動字級、行高、色塊 padding 或**組間距**時，**記得重新量測這個最擠情境**，
> 否則很容易又出現「描述多一點就被切掉」的問題。

> **窄螢幕（≤ 480px）** 空間不足，隱藏「加班」／「半夜」文字標籤，改以橘色、紫色色塊本身辨識。
