...
| ≥ 1400px | 卡片 | 56px | 177px | 19px | 2 | 214px |

**工數與加班色塊**滿版橫跨格子寬度、貼齊底部：手機版為連續的底色資料列，桌機版另有
`space-between` 佈局（左側數字、右側「加班」標籤）。

**列高的估算基準**：一個格子最擠的情況是「工時描述 2 行 + 加班描述 2 行 + 工數色塊 + 加班色塊」。
1000px 以上用固定列高（`minmax(Npx, 1fr)`），N 必須放得下這個最擠情境，否則
`.day` 的 `overflow: hidden` 會把下面的色塊裁掉。實測 1000px 需 196px、1400px 需 211px，
故分別取 200px 與 214px。

> 改動字級、行高或色塊 padding 時，**記得重新量測這個最擠情境**，
