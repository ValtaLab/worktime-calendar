...
| **深色模式** | 跟隨系統自動切換 |
| **雲端備份** | 主畫面橫幅**一鍵開啟**（不需填任何網址，Worker 端點已內置；站長自架見 `cloudflare-worker/DEPLOY.md`），開啟後每次修改**自動加密備份**（AES-GCM 端到端加密，伺服器只有密文）；**刪除 App 重裝後輸入 8 位恢復碼即自動找回全部資料**（恢復碼是唯一憑證，開通時展示大字碼請截圖保存）；內容指紋沒變不重推，切背景前自動補推 |
