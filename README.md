...
| **iOS 液態玻璃介面** | 極光漸層背景＋半透明玻璃格子（上亮下透漸層、白描邊、上緣內高光）；頂欄／收入列／編輯面板／抽屜即時模糊（backdrop-filter），格子不 blur 確保 42 格滾動順暢；月標題漸層文字、桌機格子 hover 浮起、今天格子光暈 |
| **雙擊新增** | 雙擊任一日期 → 填寫工時描述、工數、加班描述、加班時數、半夜加班描述與時數 |
| **工數三選一（可不選）** | 正常工數可選 **兼職時數（不記工數）**、**0.5 工（半日）**、**1 工（全日）**，**再點一次已選按鈕即取消**（三個都不選＝純加班日）；選「兼職時數」才會展開兼職小時卡片（選工數時收起，面板更緊湊），編輯已有兼職的記錄時自動展開 |
| **加班以小時計** | 加班、半夜加班時數皆以 **0.5 小時** 為單位遞進 |
| **工時、兼職、加班、半夜分組** | 月曆上工數（藍）、兼職時數（綠）、加班時數（橘）、半夜加班時數（紫）各佔一個色塊 |
| **描述優先** | 格子內先顯示描述，時數緊接在下方 |
| **一組一色塊** | 每格分成四組：**工時描述＋工數**（藍）、**兼職時數**（綠）、**加班描述＋加班時數**（橘）、**半夜加班描述＋半夜加班時數**（紫）。每組是一個獨立色塊，描述與數字上下排列但**共用同一個底色框**，誰對應誰一目了然 |
| **三描述同時顯示** | 同一天可同時顯示**工時描述**（黑字）、**加班描述**（橘字）、**半夜加班描述**（紫字），彼此不會互相蓋掉 |
| **七欄等寬** | 星期一至日七欄幾何寬度完全一致；週末以**日期數字變色**標示（六藍、日橘），不用底色，避免相鄰兩欄同色黏成一塊看起來較寬 |
| **香港公眾假期（紅日）** | 內置政府 1823 官方資料（2025–2027），假期日**日期數字轉紅**並在格子內顯示假期名稱（週六紅日照樣轉紅）；點開編輯面板亦顯示「香港公眾假期・名稱」；設定 → 顯示選項可**關閉顯示** |
| **主題光暗模式** | 設定 → 外觀三段選擇：**跟隨系統**（手機切換光暗模式時即時跟隨）、**淺色**、**深色**；重載不閃白，瀏覽器工具列顏色同步跟隨主題 |
| **描述記憶** | 點選描述框（工時／加班／半夜各自獨立）即顯示**最近輸入過的描述**建議，最新在前、最多 3 條，點一下即填入；重複輸入自動提到首位，歷史隨雲端備份同步 |
| **滿版版面** | 月曆方格幾乎貼齊螢幕左右邊緣（手機 2px、平板 3px、桌機 6–10px），格子越寬工數／加班色塊越大 |
| **月度統計** | 月工數、月加班時數、月半夜加班時數（有半夜紀錄時才顯示）、記錄天數 |
| **螢幕鎖定** | 選單設定 4 位數字密碼（僅存 SHA-256 雜湊、不離開裝置）；啟用後每次開啟 App 或從背景回前台都要解鎖，錯誤抖動提示、連錯 5 次鎖鍵盤 30 秒，可隨時輸密碼關閉；輸入時有**震動回饋**（按鍵輕震、成功兩短、錯誤兩重；Android 全支援，iOS 18+ 透過系統開關觸覺） |
| **收入總計** | 設定日薪、時薪（兼職）、加班時薪、半夜加班時薪後，月曆下方自動計算本月收入（工數 × 日薪 ＋ 兼職時數 × 時薪 ＋ 加班時數 × 加班時薪 ＋ 半夜加班時數 × 半夜加班時薪）；金額預設遮蔽，點一下才顯示 |
...
| 項目 | 單位 | 選項 |
| --- | --- | --- |
| **正常工時** | 工 | `兼職時數`（不記工數）、`0.5 工`（半日）、`1 工`（全日） |
| **兼職小時** | 小時 | 滾輪上下滑動選擇，每次 0.5 小時，0 至 24 小時（按時薪計） |
| **加班** | 小時 | 滾輪上下滑動選擇，每次 0.5 小時，0 至 24 小時 |
| **半夜加班** | 小時 | 滾輪上下滑動選擇，每次 0.5 小時，0 至 24 小時 |

> **舊資料自動遷移**：不論你之前用過哪一版，開啟後都會自動轉換，無需手動處理。
> - 舊版以小時記錄工時 → 換算為工數（沿用你先前設定的「1 工 = N 小時」基準）
> - 舊版以「工」記錄加班 → 換算回小時
...
月曆下方會顯示當月收入。點右上角 `⋮` → **薪資設定** 先填數字：

| 設定項 | 說明 |
| --- | --- |
| **日薪（1 工）** | 做滿 1 工對應的金額，例如 1500 |
| **時薪（兼職）** | 兼職每小時對應的金額，例如 100 |
| **加班時薪** | 每小時加班對應的金額，例如 220 |
| **半夜加班時薪** | 每小時半夜加班對應的金額，例如 380 |

計算方式：

> **本月收入 ＝ 月工數 × 日薪 ＋ 月兼職時數 × 時薪 ＋ 月加班時數 × 加班時薪 ＋ 月半夜加班時數 × 半夜加班時薪**

以 8 工、5.5 小時兼職、9 小時加班、6 小時半夜加班，日薪 $1,500、時薪 $100、加班時薪 $220、半夜加班時薪 $380 為例：

```
8 工 × $1,500 ＋ 5.5 h × $100 ＋ 9 h × $220 ＋ 6 h × $380
＝ $12,000 ＋ $550 ＋ $1,980 ＋ $2,280 ＝ $16,810
```
...
**編輯面板濃縮為一屏**：加班與半夜加班並排兩欄、時數滾輪 3 行、各段間距緊湊化——
390×844 的主流手機與 375×667 的舊小屏（另有矮屏斷點）都**不需滾動即可看到全部欄位**。

| 斷點 | 風格 | 左右邊距 | 格內內容寬 | 工數字級 | 每則描述行數 | 列高下限 |
| --- | --- | --- | --- | --- | --- | --- |
| ≤ 480px | **網格線** | 2px | 44–54px | 9px | 1 | 內容為基礎＋剩餘空間均分 |
| ≥ 700px | 卡片 | 3px | 74–91px | 14px | 2 | 245px |
| ≥ 1000px | 卡片 | 6px | 108–148px | 18px | 2 | 285px |
| ≥ 1400px | 卡片 | 10px | 164–324px | 19px | 2 | 310px |

> **左右邊距只留 2–10px**：月曆方格要「用盡螢幕寬度」，留白改由格子本身的
> 內距（`.day` 的 `padding`）承擔——那部分寬度仍屬於格子，視覺上不浪費。
> 對比先前的 10–56px，**格內可視寬度增加 4–11%**（手機 +5%、平板 +10%、
> 桌機 +6–10%）。桌機保留 6–10px 是為了不被 15px 寬的垂直捲軸壓住。
>
> 已驗證 320–430px 各寬度下，最左欄與最右欄的文字、色塊都**不會**越出
> viewport，也不產生水平捲動。

> **列高下限的取捨**：列高下限為「放得下三組描述＋三個色塊」的完整值
> （245 / 285 / 310px，v1.14 依實測最擠情境 238 / 283 / 310px 調高），
> 空間不足時**讓頁面垂直滾動**，而不是壓縮或隱藏內容。
...
右上角的小圓點徽章（v1.13 及之前的 `.badge-dot`）已於 v1.14 移除——
色塊本身就是資料標記，圓點屬於重複資訊。
...
| 調整加班時數 | 滾輪**上下滑動**，吸附置中即選中；也可點任一格或用 `↑` `↓` 鍵，每次 0.5 小時 |
| 調整半夜加班時數 | 「半夜加班」區塊的滾輪，操作同上 |
