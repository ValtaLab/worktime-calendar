...
| 斷點 | 左右邊距 | 格子寬（1600px 時） | 工數色塊字級 | 描述行數 |
| --- | --- | --- | --- | --- |
| ≤ 480px | 10px | 50px | 10.5px | 2（截斷） |
| ≥ 700px | 28px | 89px | 14px | 2 |
| ≥ 1000px | 40px | 126px | 18px | 2–3 |
| ≥ 1400px | 56px | 177px | 19px | 2 |

**工數與加班色塊**在桌機以滿版底色塊呈現（工數＝藍、加班＝橘），左右以 `space-between` 撐開：左側數字、右側「加班」標籤。色塊以 `margin-top: auto` 貼齊格子底部，描述與色塊之間的剩餘空間自動分配。
