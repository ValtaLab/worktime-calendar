...
curl -sS -X POST https://api.github.com/user/repos \
  ...
  -d "{\"name\":\"$REPO_NAME\",\"description\":\"工時月曆 PWA — 月曆介面，雙擊記錄工時與加班\",\"private\":false,\"auto_init\":false}" \
  | grep -q '"full_name"' \
  && echo "  ✓ repo 已建立" \
  || echo "  · repo 已存在，直接推送"
...
# 3) 啟用 GitHub Pages（用 Actions 發佈）
...
echo
echo "   提示：若 Pages 未自動部署，到 repo 的 Settings → Pages → Source 選「GitHub Actions」。"
